...
export function Field({ data }: { data: string}) {
    ...
    let proportionTotalSize = Math.abs(((1 - 800/(svgSize.width +1)) + (1 - 1000/(svgSize.height +1)/2)));
    ...
    const onPinchGestureEvent = (event: any) => {
        ...
        if (toolbar.zoomMode) {
            ...
            if (Platform.OS === 'web') {
                ...
                let proportionOrigin: number[] = superSvg_Field[0].map(number => number * proportionTotalSize);
                let futruProportion: number[] = superSvg_Field[0].map(number => number * newDelta);

                let distanceCompare = (Math.abs(proportionOrigin[0] - proportionOrigin[1]) + 1)/(Math.abs(futruProportion[0] - futruProportion[1]) + 1);

                //Pas de + de x10 ou /10
                if(distanceCompare < 0.1 || distanceCompare > 1.8 ){
